...
from backup_drill_check import __version__
from backup_drill_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from backup_drill_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='backup-drill-check',
        description='Audit backup plans for restore proof, RPO, and RTO gaps.',
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='backup policy text or disaster recovery notes',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"backup-drill-check: error: {exc}", file=sys.stderr)
        return 1
